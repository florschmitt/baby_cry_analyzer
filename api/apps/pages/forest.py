...
async def get_forest_page(request: Request):
    return render_template("forest.html", {"request": request})
...
async def forest_predict(request: Request, file: UploadFile = File(...)) -> Dict:
    ...
    if not await form.file_is_valid():
        return render_template("forest.html", {"request": request, "errors": form.errors})
    ...
    mfccs = extract_mfcc("temp_audio_file.wav")
